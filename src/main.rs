...
fn transcoder<P: AsRef<Path>>(
    ...
    if decoder.channels() == 2 {
        decoder.set_channel_layout(ffmpeg::channel_layout::ChannelLayout::STEREO);
    }
    if decoder.channels() == 1 {
        decoder.set_channel_layout(ffmpeg::channel_layout::ChannelLayout::MONO);
    }
...
fn main() {
    ...
    let reference_input = env::args().nth(3);
    let filter = "anull".to_owned();

    // I can't seem to figure out how to construct Format objects by hand,
    // but I can grab format information from a reference file, and apply that
    // to a raw audio file, for example:
    // a "g729" file with the name "audio.g729" will work out of the box,
    // but a "g729" file without a "g729" extension will need its format specified
    // (via, here, clumsily, a reference file)
    let reference_format = reference_input.map(|reference_input| {
        let reference_input = format::input(&reference_input).unwrap();
        reference_input.format()
    });

    let mut ictx = match reference_format {
        Some(reference_format) => {
            format::open(&input, &ffmpeg_next::Format::Input(reference_format))
                .unwrap()
                .input()
        }
        None => format::input(&input).unwrap(),
    };
    let mut octx = format::output(&"output.wav").unwrap();
